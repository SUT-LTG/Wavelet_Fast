...
for i in range(num_circles):
    ...
    circle = np.where(r <= diameter/ 2, 3*arr_rms, 0)
    #circle = 3*arr_rms*np.exp(-2*(r/diameter)**2)
    arr+=circle
...
plt.title('PetHat Wavelet Energies - w circles minus w/o circles', fontsize=11)
plt.savefig(path+'\\Output\\pethat_energy_smooth_circles.png', dpi=300)
#plt.show()
"""
